...
Валидаторы принимают опциональный `message` в конфиге. Без него сообщение будет пустым — приложение само задаёт текст ошибки (в т.ч. для i18n). Плейсхолдеры `{{0}}`, `{{1}}` заменяются на значения из `refValues`:
